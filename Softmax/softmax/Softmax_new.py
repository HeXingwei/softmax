...
class Softmax(object):
	"class softmax"
	def __init__(self,size,epo=1000,rate=0.001):
		"""
		...
		for i in range(self.epo):
			...
			for j in range(data.shape[0]):
				...
				for k in range(self.weights.shape[0]):
					if k==y:
						self.weights[k]+=self.rate*(1-h[k])*x
					else:
						self.weights[k]+=self.rate*(-h[k])*x
				self.weights=self.weights.transpose()
	...
	def softmax(self,x):
		temp=self.weights.transpose().dot(x)
		max=np.max(temp)
		temp=temp-max
		h=np.exp(temp)/np.sum(np.exp(temp))
		h=np.nan_to_num(h)
